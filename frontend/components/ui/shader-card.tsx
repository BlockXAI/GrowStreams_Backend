...
const ShaderCard: React.FC<ShaderCardProps> = ({
  ...
  useEffect(() => {
    ...
    width, height, speed, fragmentShader, autoPlay, color,
    positionY, scale, effectRadius, effectBoost, edgeMin,
    edgeMax, falloffPower, noiseScale, widthFactor, waveAmount,
    branchIntensity, verticalExtent, horizontalExtent, blur, opacity,
  ]);

  return (
    <div
      className={cn(
        "relative overflow-hidden shadow-lg border border-provn-border bg-provn-surface rounded-xl",
        className,
      )}
      style={{
        width: `${width}px`,
