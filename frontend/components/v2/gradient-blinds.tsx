...
const prepStops = (stops?: string[]) => {
  const base = (stops && stops.length ? stops : ['#10b981', '#06b6d4']).slice(0, MAX_COLORS);
...
`;
    ...
    const resize = () => {
      ...
      (uniforms.iResolution.value as number[]) [2] = 1;

      if (blindMinWidth && blindMinWidth > 0) {
        const maxByMinWidth = Math.max(1, Math.floor(rect.width / blindMinWidth));
        const effective = blindCount ? Math.min(blindCount, maxByMinWidth) : maxByMinWidth;
        (uniforms.uBlindCount.value as number) = Math.max(1, effective);
      } else {
        (uniforms.uBlindCount.value as number) = Math.max(1, blindCount);
      }
